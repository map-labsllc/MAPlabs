...
import { GROUP_PERSONAL, GROUP_SOCIAL, GROUP_WIDER } from '../../../constants';
...
export default class Influences extends React.Component {
  ...
  deleteInfluence = (groupId, keyToDelete) => {
    ...
    const newGroupInfluencesWithKeys = influencesWithKeys[groupId].filter(
      (influenceWithKey) => keyToDelete !== influenceWithKey.key
    );
  ...
  addInfluence = (groupId) => {
    ...
    if (influencesWithKeys[groupId].length) {
      previousRelationship =
        influencesWithKeys[groupId][influencesWithKeys[groupId].length - 1].item.relationship;
      previousName = influencesWithKeys[groupId][influencesWithKeys[groupId].length - 1].item.name;
    }

    const newGroupInfluencesWithKeys = influencesWithKeys[groupId].concat(
      this.uuid.getNewItemWithKey({
  ...
  updateInfluence = (groupId, keyToUpdate, newInfluence) => {
    ...
    const newGroupInfluencesWithKeys = influencesWithKeys[groupId].map((influenceWithKey) =>
      influenceWithKey.key === keyToUpdate
        ? { key: keyToUpdate, item: newInfluence }
        : influenceWithKey
    );
