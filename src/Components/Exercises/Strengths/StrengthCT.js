...
const mapStateToProps = ( state, passedProps ) => {
  console.log( "StrengthCT::mapStateToProps()" )
  ...
  if (answerRecords.length) {
    ...
    // BROADLY (the broad statement)
    // -----------------------------
    // check that first record is the 'broadly' type
    if (!answerRecords[0][IDX_EFFECT] !== EFFECT_BROADLY) {
      console.log("ERROR, first record is not 'broadly' for question.code", question.code)
    }

    previousData.broadly = answerRecords[0][IDX_PHRASE]
  ...
  console.log('StrengthsCT::previousData: ', previousData)
...
const mapDispatchToProps = ( dispatch, passedProps ) => {
  ...
  ******************************************** */
  function onUpdateStore( newData ) {
    console.log( `StrengthCT::onUpdate(${newData})` )
    ...
    if (newData.strength) {
      twoDimArrayOfString[0] = []
      twoDimArrayOfString[0][IDX_STRENGTH] = newData.strength
      twoDimArrayOfString[0][IDX_PHRASE] = newData.broadly
      twoDimArrayOfString[0][IDX_EFFECT] = EFFECT_BROADLY
